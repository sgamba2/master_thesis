...
The tracker must provide \add{an} excellent momentum 
resolution, approximately 1 MeV/c FWHM, 
in order to distinguish the monochromatic CE 
signal from the background. 
The Mu2e collaboration \del{opted for}\add{has chosen} a detector 
based on the straw tube technology.  
Its distinctive annular geometry is highly 
effective in reducing \add{the} background. 
The tracker is placed inside the 
DS, downstream from the ST, 
in a uniform 1 T magnetic field. The \del{detector}\add{tracker}  
has a modular design\add{and consists of 18 tracking stations.}\del{, consisting of basic 
...
\del{system}, focusing on the robustness and 
...
studies, investigating \del{the optimal methods for}\add{techniques of}  
...
C++ simulation,
\del{which I contributed to develop}
\add{and I contributed to the simulation code development.}
...
{'ROC buffer underflow' : the name is misleading}

\hlite{These configurations depend on the 
internal hit buffer},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the running modes do not depend on the buffer itself,
  but on whether or not it is getting filled up}
...
By studying the timing distribution\add{s} \del{and}\add{of the signal} delays 
between \add{different} channels \del{and digi-FPGAs}, we 
were able to incorporate \del{realistic}\add{relative channel-to-channel} delays into 
the simulation. The comparison 
between the data and the bit-level simulation 
demonstrated good agreement, 
validating the simulation\del{'s} accuracy in reproducing 
the expected ROC behavior \del{under}\add{in}  
\del{the} two \del{ROC buffer} modes. The agreement between 
the simulation and \add{the} experimental data was highly satisfactory,
with \add{the} deviations at \del{the}\add{a} level of \( 10^{-3} \).

The second test focused on evaluating the 
performance of preamplifiers, 
particularly looking for \del{inactive}\add{dead} channels, 
cross-talk between \add{the} channels, 
and unexpected pulse patterns. The first part 
of the test examined \add{the} channel occupancy, revealing 
...
Although \add{the} cross-talk was not fully \del{resolved}\add{addressed} at 
the time of writing, further investigations 
are ongoing\del{ with tracker experts}.
...
three peaks in their distribution. \hlite{This was due 
to timing mismatches between the pulser 
and ADC, leading to incorrect readings}. 
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not correct. The origin of the discussed effect is due to the ADCs
  and the test pulser having correlated clocks, derived from the common one.
...
\add{the} tracker calibration. Our primary objective is to perform a time calibration of 
...
\del{This would enable the distinction 
  between different particle trajectories while reconstructing helices}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{are you claiming that otherwise that is impossible ? }
...
The calibration will be performed \add{by} reconstructing the track position 
along the straw and correlating it with the arrival times. 

Due to operational constraints\del{ of the station}, such as \add{the} gas distribution, fragility, 
and space, a \del{vertical station} calibration \add{with a station oriented vertically} was considered
to be the best option\del{ for our goals}.
I focused on the reconstruction of $x_{\text{track}}$ using only the 
information \del{of}\add{about} whether a straw was crossed or not by a cosmic muon. The only 
way to deduce $x_{\text{track}}$ from this "yes-no" information is to use the geometry of the \del{tracker} station, 
which consists of a set of rotated panels arranged \del{on}\add{in} four faces orthogonal 
to the detector axis. I examined \del{the} potential 
...
muons crossing a station and then \del{the}\add{- various} hit distribuiton\add{s} \del{on the panels} 
resulting from the selection. 
The \del{request}\add{requirement} of having at least one hit per face resulted in 
a non-uniform illumination, with almost no hits 
in the central region of the panel\del{, potentially leading to 
waveform non-linearities}. 
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{illumination per se can't lead to waveform non-linearities}

To reconstruct the 3D trajectory of \del{the}\add{a} cosmic muon
\add{moving along the straight line}, at least 
...
to overlap regions of different straws. \del{The profile histogram indicated a}
\add{The} systematic \del{effect}\add{shift} in \del{determining} 
the \add{reconstructed} longitudinal position \add{could be as large as +/- 4 cm,
  with the magnitude varying along the wire}\del{, with the range extending beyond [-4, 4] cm}. 
...
The main issue with this configuration \add{is that} the stereo 
reconstruction biases for tracks with positive and negative values of $m_{yz}$
do not cancel \del{each other} out. 

\hlite{Another critical factor is the cosmic event rate, which is 
difficult to compute as it depends on various considerations, 
such as angular factors and additional unpredictable ones 
introduced by the straws' bias.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{re-read and rephrase}

Therefore, the horizontal orientation \del{should be considered}
\add{of the station is worth considering despite the difficulties
  with connecting the gas supply lines.}
\del{to optimize the calibration and achieve more 
  accurate results}.
\hlite{A new mechanical approach is currently under development to address these issues.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{explain 'new mechanical approach' ?}
