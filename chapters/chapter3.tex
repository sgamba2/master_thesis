...
\section{The Mu2e tracker}\label{geomtra}
...
Two adjacent tubes are connected to the same preamplifier PCB board. 
The latters are mounted vertically on the AMBs. 
Thus, each preamplifier PCB board contains two preamps,  
each one connected to a single straw end. 
Each panel is thus equipped with 48 preamplifier PCB boards on the HV 
side and 48 on the CAL side, 192 preamps in total.
To minimise signal reflections, the preamplifiers have a matching 300 $\Omega$ input impedance. 
