...
They provide \del{great} spatial resolution \add{which could be as good as 50 um}
and \add{high} detection efficiency at a low cost, Ref. \cite{kola}. 
...
The tube is filled with a combination of \add{a} noble gas, often Argon, and \add{a} quench gas. 
A thin sensing wire is \del{suspended}\add{positioned} along the cylindrical cathode axis. 
...
{not quite. Straw tubes - tubes with thin, often mylar, walls}
...
\del{Thinner}\add{Lower diameter} wires are typically preferred in drift tubes.
A \del{greater}\add{higher} electric field near the wire increases the amplification factor 
of the drift tube at the same voltage. Additionally, smaller \add{diameter of a signal} wire\del{s}
improve\add{s the} spatial resolution, Ref. \cite{kola}. 
...
\add{In case of a compound gas (A+B), if}\del{If} the excitation energy of $A^*$ is higher than the ionization potential of \del{another species} $B$\del{ in the gas}, the quencher, the
Penning Effect can produce ionization through $A^*B \rightarrow A B^+ e^-$.
In addition, \del{the} noble gases can
...
To compute the average number of electron-ion pairs produced by \del{an}\add{the} initial particle,
...
\del{Ionised}\add{Ionisation} electrons often have \del{reduced}\add{lower} kinetic energy,
leading to a shorter range.
\subsection{Drift of Ions and Electrons}
\del{The}\add{An} electric field accelerates free electrons and ions towards the anode and cathode 
along the field lines. \hlite{As charges accelerate, they scatter}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{what is the ``charges''? }
...
\hlite{charges} move uniformly along the electric field. This is referred to as the drift velocity of the 
...
and it is typically \hlite{$\mathcal{O}(10^3)$ cm/ns},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
...
\begin{figure}[!h]
    ...
    \label{fig:drift}
\end{figure}
\hlite{Electron drift velocities in drift tubes are typically $\mathcal{O}(10^6)$ cm/ns.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{and what is the speed of light in these units? }
...
\hlite{Free electrons rapidly multiply, }
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{are they also able to rapidly add and subtract? }
%
resulting in an avalanche. In a drift tube\del{s}, where the electron mean free path is about the order of $\mu$m, 
...
for ionisation to \del{occurr}\add{occur}. The energy distribution depends on the electric field which is position dependent. 
...
the gain of \hlite{a drift tube with a particular shape}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{a drift tube has a shape of a tube, right? so what are the other shapes referred to?}
%
and gas mixture. 
...
\begin{figure}[!h]
    ...
    {The dependence of \hlite{particle gain}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{how the particle gain is defined ?}
%
...
Three rotated panels form a face, and two faces rotated of \del{60°}\add{30°} form one plane, Figure \ref{fig:trkpanel} (top right-left). 
\hlite{Two planes rotated of 60° make one station},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{this is not correct, don't claim 60 deg}
%
...
The dual-layer geometry was chosen \hlite{since it allows to determine 
  easily which side of the sense wire the particle passes through and thus solve the left-right ambiguity.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{don't make up the reasoning. Rephrase.}
%
...
At the moment of writing this Thesis, almost all \del{the} tracker planes have 
already been assembled at Fermilab Lab3 Facility and \add{the} station assembly \add{was}\del{is now work} in progress, 
...
The panel has two sides, \del{named}\add{the} high-voltage side (HV-side), 
which handles the high-voltage supply distribution to the straws, and 
\add{the} calibration-side (CAL-side), that contains \del{a}\add{the} circuitry
\del{that can}\add{to} inject \add{the} calibration pulses\del{ to the channel}.
...
\del{Moreover, the}\add{The} AMBs and the DMB \add{also} handle 
low voltage distribution and the AMB on the HV side \del{is responsible for 
distributing}\add{distributes} the high voltage to the \del{straw anode} wires.
The low voltage power supply is connected to the panel through the KEY board which 
contains also an optical fiber link and a JTAG \del{inferface for communication}\add{connector}.
The \del{two} boards are also equipped with sensor\add{s} to monitor environmental parameters, 
...
\hlite{There are some differences between the preamps on the CAL and 
  HV sides.
The CAL preamp can inject a calibration pulse into 
...
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{all preamps are the same and don't inject anything anywhere, different are the AMBs}
%
...
Figure \ref{fig:drac} shows a picture of the board: a \hlite{battery of comparators 
and ADCs}, three DDR3 \del{memories}\add{memory chips} and three Field-Programmable Gate Arrays (FPGAs) 
are clearly visible. 
...
\begin{itemize}
    ...
    \item In the DRAC, the two biased signals are fed to zero-crossing comparators, 
      which generate \hlite{square pulses} if the signals are above their respective thresholds;
    \item The \hlite{squared signals} are transmitted to 16-bit TDCs implemented 
      \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
      {decipher 'square pulses'}
      %
    in \add{the} firmware in the digi-FPGAs. Timing digitization \del{is performed at the rate 
    of}\add{bin is} 20 ps\del{ per count}, including the determination of the arrival time and the 
    time over threshold.  \hlite{In addition to the drift time, the TDCs 
    measure the time difference between the two signals received from the 
    same straw to dermine the hit position along the straw.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the TDS's don't do that, the time difference is calculated in the reconstruction software }
  ...
  \item \del{The two signals}\add{Signals} from the two ends of the same straw
    are also \del{added by an integrator.}\add{summed up, the} \del{The total} sum is digitized by a 10-bit 
    ADC at 40 MHz and \del{then} transmitted to the digi-FPGA;
    \item The digi-FPGA creates one data packet for each hit \del{based on}\add{containing} the 
    TDC and ADC information;
    \item The data packets are transferred to the ROC \ref{DRAC}, and 
    temporarily stored in the DDR3 \del{memories}\add{memory} for later access by the DAQ system. 
    ...
    digi-FPGAs, buffer data and transfer them to DAQ. \hlite{They continuously stream out 
    the zero-suppressed data collected between two proton pulses to the DTCs, }
    Ref. \cite{GIOIOSA2023167732}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{a) explain who are 'they' ? \\
  b) DTCs have not been introduced yet}
%
  \hlite{The buffer stage is fundamental, 
  ...
  \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
  {this is wrong. The buffering is needed because the DAQ is not able to handle the instantaneous 
...
The Readout Controllers of all \del{the} detectors are shown 
...
Based on these markers, the Command Fan-Out (CFO) module \del{within the 
...
and \del{subsequently} transferred to \add{the} long-term storage\del{ for future offline 
  analysis}. Calculations estimate a data rate of 35 GBps \hlite{(with a 30\% overhead)}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{explain overhead}
%
...
In addition to \add{the} detector data, the DTCs also handle the acquisition of 
...
specified PCB \del{nodes}\add{boards}, high voltages, and channel threshold settings across 
...
The tracker must operate in an ambient vacuum (< $10^{-4}$ Torr) \del{.
It should}\add{and} be able to withstand a rate of 5 MHz per straw (highest rate straw) 
500 ns after the spill. \del{This is
  for background studies. The nominal experiment live time starts 700 ns after the spill.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not directly unrelated to the tracker performance requirements}
