...
\section{Drift tubes}
\del{Gas detectors are capable of measuring charged particle coordinates.
They}\add{Gaseous detectors} provide spatial resolution which could be as good as 50 $\mu$m
...
The sensitive elements of the \del{detector}\add{Mu2e tracker} are the straw tubes
with a diameter of 5 mm and a \del{variable} length \add{varying in} between 40 cm and 110 cm, 
filled with a 80\%:20\% Ar:CO$_2$ mixture at 1 atm.  
The detector has a modular design, \add{and it is} made of basic elements named $Panel$, $Face$, 
...
and has \del{a}\add{the} total length of 3.2 m and \del{a}\add{the} diameter of approximately 1.7 m.
...
Figure \ref{fig:trkpencil} shows \del{the edge}\add{an end} of a straw tube, compared to a pencil.
\begin{figure}[!h]
    \centering
...
All \del{the} straws have the same diameter of 5 mm, \del{but their}\add{and the straw} length 
\del{is}\add{varies} between a minimum of 334 mm and a maximum of 1174 mm, 
...
are joined to both ends of the straw using \add{a} silver \del{epoxied}\add{epoxy}.
    \begin{figure}[!h]
        \centering
        ...
        allows \add{the} gas flow into and out of the tube. The insert 
...
manufacturing tolerances and allows for \del{straws}\add{the straw} expansion
due to \add{the} gas pressure. This requires each straw to be self-supporting across its 
...
of the panel edge is shown in Figure \ref{fig:strawtubes}.
            ...
            \add{The overpressure is essential for the straw mechanical stability.}
...
for transmission to the Data Acquisition System (DAQ)\del{ of the straw-tube signals. 
In particular, each single}\add{. Each} straw provides two hit times and one waveform, 
which are necessary to reconstruct the hit position along the straw \add{and the hit charge}, 
...
\begin{figure}[!h]
    ...
    \label{fig:flowfee}
    \end{figure}
\del{All the}\add{The plane} front-end electronics is installed on \del{multiple}\add{several} Printed Circuit Boards 
...
Signals from the straw tube \del{channels} are first read 
out from both ends by the pre-amplifiers (preamps).

\del{Both sides}\add{Each side} of the panel \del{have}\add{has} one Analog Mother Board (AMB) and one Jumper board, 
...
The AMBs and the DMB also handle \add{the} low voltage distribution,
...
In addition, the frontend components were \del{chosen}\add{qualified} to sustain high level of radiation.

The first step of signal processing is the signal pre-amplification performed 
by \add{the} preamps mounted on two AMBs placed on the two lateral sides of the panel. 
...
To summarize, each straw tube is an independent detector channel \del{equipped}\add{instrumented} with:
\begin{itemize}
    ...
    \item one \hlite{High Voltage} feed.
...
All \del{the} 96 straw tubes are read out from both ends. 
Two adjacent \del{tubes}\add{straws} are connected to the same preamp PCB board. 
...
the Mu2e DAQ system. It also controls all \del{the} panel operations. The schematics of the 
...
each of them receives \del{the} data from 48 straw channels, performs data 
monitoring, buffering and assembles \add{the} data packets which are then 
...
\begin{itemize}
    ...
    \item In the DRAC, the two biased signals are fed to zero-crossing comparators, 
      which generate \hlite{square signals} if the signals are above their respective thresholds;
    \item \hlite{The squared signals}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{explain 'square signals' }

      are transmitted to 16-bit TDCs implemented 
  ...
  \item Signals from the two ends of the same straw
    ...
    \del{The buffer stage is fundamental}\add{Buffering is necessary}, since the DAQ is not
    ...
    \hlite{programmable nature of the digitizer, ROC and DAQ.}
    \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{'programmable nature of the DAQ' sounds awkward, rephrase }

\end{itemize}

\subsubsection{The tracker data format}
\del{Each event (hit) is composed of a}\add{A hit} data packet \del{having}\add{has}
a fixed length of 256 bits (32 bytes).
The packet \del{has the following} structure \add{is as follows}:
\begin{itemize}
...
Requests. \del{Event data from all detectors is collected at}\add{Events
  which passed the trigger are sent to} the Data Logger 
...
\add{the} specified PCB boards, high voltages, and channel threshold settings across 
...
Here a \del{resume}\add{summary} of the requirements that the tracker must satisfy to ensure the 
success of the experiment is presented \cite{trkreq}.
A momentum resolution \del{less}\add{better} than 180 keV/c for a 105 MeV/c electron is needed in the nominal
