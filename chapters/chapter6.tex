...
the distribution of the hits across the panel should be as
uniform as possible. On the other hand, this is 
not what we obtain, due to the station geometry
and selection cuts we applied. 
Figure \ref{fig:illumination} shows the resulting hit distribution for panel 0 of
plane 0, after applying the selection cuts outlined in Section 
\ref{eventselection}, which corresponds to the panel in 
the upper right part of Figure \ref{fig:coordinate}. 
We picked this panel as one possible example, the 
distributions for all the planes in both planes are similar, as
...
\end{figure}
Figure \ref{fig:illumination} shows a non-uniform, or spotty, 
panel illumination. This is due to the 
