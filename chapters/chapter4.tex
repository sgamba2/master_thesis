...
The second one describes the initial data quality monitoring of the tracker preamps. 
  ...
  followed by the event readout.
    ...
    select one between \add{the} two ROC operational modes:
    \begin{itemize}
    ...
    that represents the distance between add{the} \hlite{two consecutive 
      proton pulses (called heartbeats - HB's),}
    \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
    {proton pulses and heartbeats are two different things, a data packet
      corresponding to a proton pulse is the EWM - event window marker}
...
Figure \ref{fig:2} (Bottom) \del{shows a zoom}\add{zooms in} on the 
...
\label{fig:3}
\end{figure}
\subsection{The \textit{regular} readout mode}
...
A straightforward procedure was adopted to estimate the baseline: 
...
In some cases, a lower baseline value was observed. 
