...
The second part describes the initial data quality monitoring of the tracker preamps. 
}
  ...
  followed by the event readout.
    ...
    proton pulses (called heartbeats - HB's) and separated by 
    ...
    The relative timing offsets between different channels of 
...
%
  This procedure 
    is repeated for all channels, in a fixed order.
...
\item
  ...
  the pulses are read out in the same order as in the firmware;
\item
  the readout \textit{continues} until all simulated hits 
  have been ``read out'', or the threshold of 255 hits has been reached.
\end{itemize}
...
The bin ordering in the histogram corresponds 
...
phenomenon arises from the triggering 
on the trailing edge of the 4 $\mu$s long input pulse. 
