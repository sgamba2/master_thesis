...
Finally, the last section provides a brief overview of the DAQ performance studies conducted with the external CFO.
  ...
  \section{Description of test stand setup}\label{des}
    ...
    \begin{figure}[!h]
        ...
        \label{fig:TS1}
        \end{figure}
    Most of the data were taken operating in the second mode, with digi FPGAs.
    ...
    \label{fig:3}
    \end{figure}
    \subsection{Validation of ROC readout test stand}
...
In the following sections, the results of the data collection are compared with the simulation.
  ...
  \begin{figure}[!h]
    ...
    \begin{figure}[!h]
      ...
      \label{fig:delay2}
    \end{figure}
\subsection{\textit{ROC buffer overflow} mode}
...
\label{fig:3}
\end{figure}
\subsection{The \textit{regular} mode }
...
The negative tail of the waveforms is caused by a differentiating circuit that shapes a signal pulse 
...
The test described in this section uses the CFO in a mode expected to be implemented 
during data acquisition. In the test, sequences of HBs \ref{des} were generated, separated by 
\( T_0 \), and sent to the DTCs. The test procedure was fully programmable. 
...
In both modes, the CFO (or the DTC, in \textit{emulated CFO} mode) 
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=\textwidth]{figures/png/Screenshot_20240627_144108.png}
  ...
  The following packets (black) represent the ROC headers, six in total. 
  The orange and red squares indicate the payload length (the number of packets after 
  the ROC header, including the ROC header) and the ROC ID. The blue square represents 
...
\end{figure}
An increasing number of events were generated


do see that results depend on the source of timing synchronization
timing synchronization becomes more and more likely to be one of the major issues
