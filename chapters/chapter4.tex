...
The second \del{one illustrates}\add{part describes}  the initial data quality monitoring of the tracker preamps. 
Finally, the last section provides a brief overview of the DAQ performance studies.
  ...
  chain of \del{an entire}\add{a} 96-channel\del{s} tracker panel \del{:}\add{.} 
  48 channels connected to the digi-FPGA-1 and 
  48 channels to the second one. 
  Each channel \del{represents}\add{corresponds to} a tracker straw.
  \del{which was absent from our}\add{at the}  test stand\add{, the straws were not installed}.

  The DRAC board was connected via optical 
  fiber to one DTC installed in DAQ computer (mu2edaq09).
  The DTC was programmed, for most of \del{our}\add{the} 
  tests, to emulate the CFO 
  functionality to send the request to the 
  ROC to perform the readout of one event.
  \del{The DTC emulated the CFO by sending a request 
  for one event, which}\add{The event request} was immediately 
  followed by \del{reading} the event\add{ readout}.
    ...
    itself (user-defined) without reading \add{the} digi-FPGAs;
    \item  \textbf{ROC external mode}: the ROC receives data 
    from the digi-FPGAs. This \del{will be}\add{is} the mode \add{to be} used during 
    \del{Mu2e}\add{the} data-taking.
    \end{itemize}
    ...
    \begin{figure}[!h]
        ...
        \label{fig:TS1}
        \end{figure}
    Figure \ref{fig:timing} shows the \add{readout} timing diagram 
    for \del{the readout of} one detector channel.
    There are two important parameters: 
    the Event Window width ($T_{EW}$) shown in red, 
    that represents the distance between two consecutive 
    proton pulses (called heartbeats - HB's \del{-}\add{and} separated by 
    ...
    \del{For the purpose of these tests and DAQ commissioning, 
    the system allows for the flexibility to vary 
  }\add{in the process of testing,} the $T_{EW}$ \add{has been varied}
  between 700 ns to 50  $\mu$s. 

    The ROC firmware has an internal hit buffer 
    which stores up to 255 hits \add{per event}, \del{which is} adequate 
    \del{given the expected hit multiplicity 
    in the detectorsat run time}\add{for the expected run time occupancy}. 
    Depending on $T_{gen}$ and $T_{EW}$, the data 
    taking can proceed in two different modes:
    \begin{itemize}
      \item \textbf{\del{The ROC} regular mode}: the total number of hits 
      within the event window is less than 255.
      In this case the ROC hit buffer doesn't get filled up 
      and the total number of 
      hits may vary from one event to another \del{one}. It is 
      expected that the number 
      of hits will not exceed the threshold of 255, 
      with an average of approximately 12 hits;
    \item \textbf{\del{The ROC} overflow mode}: the event window 
    is large enough, 
    ...
    The relative timing offsets \del{among}\add{between} different channels of 
    ...
    when \add{the} DRAC is powercycled.


    \hlite{Depending on the event window and the start of the pulse 
    sequence, the first channel in the readout sequence 
    ...
    which then begins its own read out phase.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not clear what you want to say, clarify}
%
  This procedure 
    is repeated for all channels, \del{following}\add{in} a fixed order.
\section{Validation of ROC read out and buffering}
The first test \del{I} performed \del{has been}\add{was} to verify the correct performance of 
ROC buffering.
During \del{these}\add{the} test, a single ROC was connected to \del{the}\add{a} DTC. 
Data were collected with digi-FPGAs pulsed by their internal pulsers, 
\add{with}\add{and} the ROC set in the external mode. 
\del{Concering the}\add{The} digi-FPGAs pulse generators\del{, they can} operate\add{d} at two specific frequencies, 
...
ROC's \del{digital} readout logic \del{allows to}\add{can} be 
...
The main steps of the simulation are \del{the following ones}\add{as follows}:
\begin{itemize}
\item
...
\item
  ...
  the pulses are read out \del{following a predefined channel readout ordering;;}
  \add{in the same order as in the firmware}
\item
  the readout \textit{continues} until all \del{the} simulated hits 
  have been \del{included}\add{``read out''}, or the \del{maximum} threshold of 255 hits has been reached.
\end{itemize}
...
be readout in each digi-FPGA}\add{in the readout sequence}: channel 91 for 
digi-FPGA-1 and \add{channel} 94 for digi-FPGA-2. 
Figure \ref{fig:delay1}(\ref{fig:delay2}) shows an example of 
distribution of the \del{offset}\add{time residuals} between the \add{pulses in a} reference 
channel 91(94) and a randomly selected channel, 0(44) for 
digi-FPGA-1(digi-FPGA-2). 
The mean offset was determined with a \del{simple} 
Gaussian fit of the distribution. 
This procedure was repeated for all channels 
relative to each digi-FPGA.
All \del{the} measured offsets were of the order of a 
few ns and were \del{inserted}\add{included} in the simulation\hlite{ because 
...
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the sentence too long, break it down into several ones}
...
the $T_{EW}$ was set to 50 $\mu$s and $f_{gen}$ to $\sim$60 kHz\del{, 
which}\add{That} corresponds to 
\del{a}\add{the} time distance between two consecutive pulses of 
...
between the \add{start of the} readout window and the generated pulses sequence.
...
however the bottom one shows some \del{unexpected}\add{non-trivial} features.
\begin{figure}[!h]
...
\del{can be found}\add{comes} from Figure \ref{fig:2} (Top), 
which shows the \add{channel} occupancy as a function of \add{the} channel number 
(red is for data, blue for simulation).

The \add{bin} ordering \del{of the bins} in the histogram corresponds 
to the channel readout ordering\del{: this}\add{. That} means that 
the first bin on the left corresponds to the first 
\del{readout} channel and the last one on the right 
corresponds to the last \del{readout} channel\add{in the readout sequence}.
In other words, channels on the left 
are at the beginning of the readout sequence 
and \del{are, in these conditions, always 
  completely readout}\add{all hits from those channels are always fully read out}.
On the other hand, channels on 
the right are in the 
later part of the readout sequence and may 
not have all the hits \del{readout}\add{read out}, 
since the ROC buffer may \del{be in overflow}\add{get filled up}.
  \begin{figure}[!h]
    \begin{subfigure}[b]{\textwidth}
        \centering
...
hits \del{of}\add{in} channel 0 of \add{the} digi-FPGA-1. As expected, 
the majority of events 
has 3 hits, with a tail of 4 hits. 
This provides an explanation \del{for}\add{of} Figure \ref{fig:2} (Top).
\begin{figure}[!h]
\centering
...
spanning \add{over} the last 9 bins.
This pattern \del{can be}\add{is} generated by \del{adding together the 
following}\add{the} three \add{following} types of events \add{added together}.
\begin{itemize}
  \item \textbf{\add{channels} 0-68\del{th read out channels}}: 
  ...
  total of 255 hits. All \del{the} hits sent to the following 
  channels are lost.
  \item \textbf{\add{channels} 0-75\del{th read out channels}}: 
  ...
  \item \textbf{\add{channels} 0-85\del{th read out channels}}: 
...
channels and the \add{channel} \textit{occupancy} distribution shown 
...
Given \del{this}\add{the} choice of $f_{gen}$ and $T_{EW}$, 
...
data and the MC distributions is at \del{the}\add{a} level of $10^{-3}$, 
...
As mentioned in Section \ref{preampss}, \hlite{the 
preamps on the CAL side 
include circuitry that can inject calibration pulses into the channel.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{this is not correct - all preamplifiers are the same, however the CAL side FPGA
  is programmed to manage the generation of the test pulses }
...
and an undefined \add{ROC} link ID\del{ between ROCs}.

\subsection{Test 1: channel occupancy versus channel ID}\label{nhitvschid}
The first \add{conducted} test \del{I conducted was to} examine\add{d} the channel occupancy. 
...
It was also essential to detect \del{any}\add{the} cross-talks (unwanted 
...
\del{For the previous reasons, we setup our system 
  to pulse one every eight channels.}
\add{The implemented preamplifier pulsing scheme allows to pulse every 8-th channel.} 
\del{To scan}\add{Scanning} all \del{the} 96 channels of the panel, 
\del{we performed multiple}\add{requires eight} runs \del{choosing each time 
a different sequence of channels}\add{taken with the different first pulsed channel offset}.
...
In \del{the} case of the dead channel, the problematic preamp\add{lifier}
was replaced. \del{In the second case, to 
investigate this more thoroughly, }\add{To investigate the second case,}
\del{I made the}\add{a} distribution of the time difference 
$\Delta t$ between consecutive pulses 
(Figure \ref{fig:deltatnhits}) \add{has been made}.
Since the \del{pulser provides pulses at 
the frequency of}\add{the pulse frequency is} 50 kHz, the $\Delta t$ 
...
\del{In particular, the analysis}\add{Analysis} of the 
\add{hit} waveforms \del{of the hits} in the 4 $\mu$s peak 
...
\add{a} cross-talk \del{among}\add{between the two} channels. 
...
At the \del{moment}\add{time} of writing this Thesis, the 
...
\end{figure}
\del{The ouput of the test was reported to the tracker panel 
  experts, and they are still working on it.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the stricken out sentence repeats the one before it}


\subsection{Test 2: analysis of the readout pulses waveforms}\label{wf}
\del{The test described in this section involved}\add{This section describes} the study 
  of the reconstructed waveforms from \add{the} charge injection.
The sampling frequency of the ADC is 40 MHz\del{, which resulted 
in a}\add{corresponds to the} waveform bin width of 25 ns (Section \ref{DRAC}). 
\del{The maximum number of samples for one waveform is 30.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{science fiction}
...
of \add{a} reconstructed waveform (RUN105421). 
The reconstructed waveforms shown below 
have \del{already had} the baseline subtracted. 
...
waveform should have a negative tail\del{, contributing to 
its overall shape}.
The \del{negative} tail is generated by a 
...
The filter does not affect the \del{rapid} leading edge 
of the pulse because the time constant of the \del{differentiator}
\add{differentiating circuit} is \del{not small}\add{large} compared to the rise time.
...
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{replace 'pedestal' --> 'baseline' }

A straightforward procedure was adopted to estimate the \del{pedestal}\add{baseline}: 
...
\del{we simply did not used the bins with the negative peaks and estimated the pedestals with 
  the remaning samples}
\add{the problematic samples have been identified and excluded from the baseline estimate}.
\begin{figure}[!h]
  \centering
...
in channel 70 was observed\del{, 
leading }\add{. That lead} to a different \add{from expected} $\Delta t$ distribution 
between \add{the} hits \del{than expected}. 
...
phenomenon arises \hlite{from the erroneous 
  generation of 4 $\mu$s long pulses},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{nothing wrong with the generation, just triggering on the trailing edge of the 4us input pulse.}
%
...
\hlite{One potential solution to address 
...
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the solution is likely the same - replace the preamplifier}
%
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.65\textwidth]{figures/pdf/wf_ch50_1.pdf}
  \caption[An inverted waveform.]{Inverted waveform of the \del{50th channel}\add{channel 50}.}
 \label{fig:inverted}
\end{figure}
...
the number of hits. Moreover, a \del{clear}
correlation between the positive charge and pulse 
height peaks \del{is observable}\add{has been observed} (Figure \ref{fig:chvsph}). 
\begin{figure}[!h]
  \centering
...
problematic channels and check the \add{response} uniformity
\del{of channel responses}\add{across the channels}.
