...
\textit{In this Chapter, I present the initial results of the tracker DAQ commissioning. Before reading out the detector, 
  ...
  \section{Description of test stand setup}\label{des}
    ...
    to the DTC \ref{tdaqtra} installed in the DAQ computer, called mu2edaq09. The test stand included 96 channels in total.
    ...
    During the test described in Section \ref{MonteCarlo}, the data were taken operating with digi FPGAs, pulsed by their internal pulsers.
    ...
    Multiple tests were conducted to verify the correct behavior of the ROC. 
    During these tests, only one ROC was connected to a single DTC. The DTC 
    emulated the CFO by sending a request for one event, which was immediately followed by reading the event.
