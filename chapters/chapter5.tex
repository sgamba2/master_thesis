...
The horizontal orientation of the station allows for an unbiased reconstruction, as cosmic rays are distributed according to cos$^2\theta$ and are predominantly vertical.
...
\section{Reconstruction of the longitudinal position}\label{genplane}
...
The station was simulated in an extracted position, meaning it is placed outside the solenoid. The magnetic field of the Mu2e hall was turned off. 
...
with fewer than three straw hits per panel. This criterion was chosen to minimize the error in position reconstruction.
...
The illumination is spotty and non-uniform. This unusual illumination is due to the panels' overlap areas being located at the extreme edges. 
The overlap selects specific cosmic directions in the $y$-$z$ plane. 
To better visualize the selected directions, the Monte Carlo $m_{yz}=\Delta y /\Delta z$ distribution has been plotted.
There are no tracks corresponding to the $m_{yz}\sim 0$, which are horizontal tracks. The vertical tracks (the most probable ones), 
which correspond to $m_{yz} \sim \infty$, are also not selected by our algorithm. This results in a very low rate too, that will be 
investigated better in Section \ref{ratetracker}. The most selected directions are those with $|m_{yz}| \sim 1$, which represent cosmics with 
an angle of approximately 45°.
\begin{figure}[!h]
    \centering
    \includegraphics[width =0.8\textwidth]{figures/pdf/myz.pdf}
    \caption{Monte Carlo $\Delta y /\Delta z$ cosmics distribution.}
    \label{fig:myz}
\end{figure}
...
\subsubsection{Rate}\label{ratetracker}
The rate of generated events is calculated by integrating the flux on the solid angle and on the generation plane described in Section \ref{genplane}:
\begin{equation}
    \int \frac{\text{d} I}{d E_\mu \text{d} \Omega \text{d} t \text{d} S} \text{d} S_{\perp} \text{d} \Omega
    \end{equation}
...
\begin{equation}
    R_{straw}=R \ \frac{1}{(N_{faces}\cdot N_{panels}\cdot N_{straws} )}
\end{equation}
...
$ComboHit$s from different faces within the same plane. The $z$ coordinate of the $StereoHit$ is the average of the $z$ coordinates of the $ComboHit$s.
The situation is reproduced in Figure \ref{fig:stco}.
\begin{figure}[!h]
    \centering
    \includegraphics[width =0.7\textwidth]{figures/png/Screenshot_20240601_195651.png}
...
\begin{equation}
 ...
 R_{x,i}&=(m_{yz}\cdot z_i+q_{yz}-q_{xy})/m_{xy}
\end{aligned}
\end{equation}
