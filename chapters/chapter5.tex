...
designed \del{for being operated}\add{to operate} in a 
...
In the following section, \hlite{simulation performed to reconstruct cosmic tracks}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{re-read, rephrase}
\hlite{will be described with a vertically oriented station},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{need to explain how a simulation can be described WITH a station}
...
\del{by}\add{at} Los Alamos National Laboratory (LANL). CRY \cite{Hagmann2007CosmicraySG} functions as a generator for air showers 
...
\hlite{CRY was chosen for its straightforward implementation}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{you probably wanted to say smth else: implementation is smth hidden from users. Rephrase}
...
simulation also incorporates \del{the} effects of the external neutron shield surrounding the DS 
...
reconstruction, it is important to define the coordinate system\del{, 
  as}\add{. As} shown in Figure \ref{fig:coordinate}\del{. The}
\add{, the} $z$ axis is parallel to 
the DS axis, while the $y$ axis \del{corresponds to the vertical axis}\add{is directed vertically}.
\begin{figure}[!h]
    \centering
...
face and between different faces, if \del{this}\add{a} particle
...
\del{lying on}\add{in} a plane $z = z_i$, where $z_i$ is the $z$ 
...
\hlite{To achieve precise calibration, the hits should 
...
This could lead to \hlite{waveform non-linearities}:
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{a wafeform is a waveform, it is all non-linear, not a straight line,
  need to defined what you call the 'waveform non-linearity' ?}
...
the other end. \hlite{This wider portion of the signal}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{'wider portion of the signal' unexplained}
...
\section{Reconstructing muon tracks}\label{reconstruction}
...
equations as the\hlite{ $x-y$ $ComboHit$s equations}:
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not clear what is the difference between ComboHits and ClusterHits, explain}
...
the reconstructed coordinate and the \del{Monte Carlo}\add{simulated} position 
on the panel frame.
...
Figure \ref{fig:bias} shows the distribution of the longitudinal bias $\Delta x$
determined as the difference between the projection 
of the reconstructed track on the panel and \del{Monte Carlo}\add{its simulated} coordinates. 
...
so \add{for different straws} the bias could be very different\del{ between straws}.
...
\del{for}\add{of} the calibration is expected to become challenging. 
...
\begin{itemize}
    \item the selection criteria in Section \ref{eventselection} select cosmic muons with orientations that affect panel illumination. 
      The illumination is non-uniform, and moreover, there are almost no hits in the central region of the panel, which could result in \hlite{waveform non-linearities};
      \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{'waveform non-linearity' undefined, the second time}
