...
the rate must be scaled by two factors compared to the horizontal orientation. 
The first factor, approximately $1/\cos^2\theta \sim 1/2$, accounts for the flux 
dependence on the angle, and the second one, $1/\sqrt{2}$, accounts for cosmic 
rays striking the station at a 45° angle.  
...
which corresponds to a total of $N \sim 2.88 \times 10^3$ cosmics hitting one station. 
...
coordinate of the panel, for panel 0, taken as an example.
...
between the projection of the reconstructed track on the panel 
...
\begin{figure}[!h]
    ...
    \label{fig:bias}
\end{figure}
The 2D distribution of the longitudinal bias $\Delta x$ 
...
to visualize the profile histogram of the bias versus 
...
error on the mean. 
The profile is reported in Figure \ref{fig:profile}. 
It reveals a systematic effect in determining the 
longitudinal position within a range exceeding 
[-4,4] cm. The initial portion of the histogram (CAL side) 
...
rotated by 180° about the vertical axis (the same applies for the other faces), 
three pairs of panels from this two faces form this angle 
(Figure \ref{fig:coordinate}). In this situation the bias is smaller 
...
of the bias. Under these conditions, achieving the required accuracy 
for the calibration is expected to become challenging. The main issue 
with this plot is that, in the vertical orientation, the stereo 
reconstruction biases for tracks with positive and negative values of 
$m_{yz}$ do not cancel each other out.

In fact, considering the vertical orientation and imagining two cosmic rays 
with opposite $y-z$ orientations, since cosmic rays come from above and travel 
downwards, the first one would hit plane 0 and then plane 1, while the second 
...
histogram for a single panel.
In the horizontal case, the same systematics apply as in the vertical case. 
However, in this scenario, both cosmic rays would first hit plane 0 and then 
plane 1, and since most muons have a vertical trajectory, they would hit mostly 
...
The systematics brought by this type of reconstruction arise 
come from the fact that $m_{yz}=\frac{\Delta y}{\Delta z}$ 
...
are not possible in reality. No horizontal muons are in the sample, as can be seen 
...
\begin{figure}[!h]
  ...
  \section{Conclusions}
  This chapter discusses the initial steps towards the tracker 
  calibration.
  The ultimate goal is to perform a time calibration of 
  the first assembled station of the tracker using cosmic muons, aiming for a longitudinal hit 
  position resolution better than 4 cm. 
...
\begin{itemize}
    ...
    \item the rate of cosmic events is $R \sim 380$ Hz, and the expected duration of 
    the calibration is about 6 days without interruptions, 
    which is excessive for the simple calibration that needs to be performed;
...
For this reason, a new mechanical solution 
