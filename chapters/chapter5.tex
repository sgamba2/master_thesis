...
the time calibration is \del{enabling}\add{to enable} the 
...
information is to \del{use}\add{rely on} the tracker station 
geometry\del{, which is made of a set of rotated 
panels lying on four faces orthogonal to 
the axis of the detector} (Section \ref{reconstruction}).
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{geometry is not made of anything}
...
Figure \ref{fig:gassystem} \hlite{shows a schematic representation of the fully assembled tracker.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the figure shows smth very different from a fully assembled tracker}

\del{Clearly, the entire}\add{The} detector, its infrastructures 
and services, and, consequently, the stations have been
designed for being operated in \del{what we call}\add{a} 
$vertical$ $orientation$\del{, which is the orientation of
the detector in Mu2e}. This has now an impact 
...
cosmic muons is the $horizontal$ $orientation$\del{, that}
\add{That} facilitates unbiased 
...
\hlite{The Mu2e simulation shows that to achieve the 
desired momentum resolution, it is necessary 
to have a resolution on the hit position along 
the straw better than 4 cm.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{momentum resolution is NOT directly related to the resolution along the wire. Rephrase}

In the following 
sections, I will 
introduce the \hlite{simulation performed to reconstruct cosmic tracks }
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{rephrase}
%
with a vertically oriented station, with the aim of understanding 
potential biases in determining the longitudinal position\del{ due to 
...
conduct the initial calibration with \hlite{the system in a vertical orientation}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{what is 'the system'? .. during the data taking there will be more than
  one station available, for the VST, there is only one }
...
\del{For this reason, in}\add{In} this Chapter, I \del{will} report on a simulation study 
which shows that the vertical orientation introduces a significant bias on 
the $x_{track}$ which \del{does not allow to perform}\add{significantly complicates}
...
\begin{itemize}
    ...
    for horizontal detectors \del{with}\add{and} a mean
    \add{muon} energy of $\sim$4 GeV \cite{muonflux}) 
...
The muon flux at sea level is usually described by the \del{standard} Gaisser\del{'s} formula \cite{guan2015parametrization}:
\begin{equation}
    ...
    \end{equation}
where $E_\mu$ is the muon energy and $\theta$ is the \add{muon} polar angle\del{ of the muon}. 
...
\hlite{and for energies $E > \frac{100}{\cos \theta}$ GeV.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{valid only for E> 100 GeV? - cross-check}

A modified version of the standard Gaisser formula, called Gaisser\add{-}Tang model, is used to account for low energy and large zenith angle effects \cite{guan2015parametrization}:
...
for its straightforward implementation\del{ and compatibility with GEANT4}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{ a) this is science fiction - there is no CRY-G4 interface in CRY
  b) add reference to CORSIKA
}
...
\hlite{have energies around $\sim$ 10 GeV},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{take a look at the cosmic muon spectrum and revisit this statement}
...
magnetic field \del{intensity equal}\add{set} to zero. 
...
perform this study, \hlite{we selected only those events where a cosmic 
  muon crosses one single station}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{wouldn't such a selection bias the angle distribution? }
%
\del{One station is made of two planes, one plane is made of two faces, and one face is
...
Although the station has \del{globally} four layers, 
the panel geometry does not provide a uniform
\del{volume} coverage: there are regions of the 
detector where a particle with a trajectory orthogonal
to the panel \del{may cross}\add{crosses} four, three, two or even only one 
...
In this parametrization, the straws \add{of a single panel (?) } are all parallel 
in \hlite{the $y-z$ plane.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{if the two lines are parallel, that doesn't depend on a view direction}
...
\hlite{more than one hit per face is needed}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not sure why more than one hit per face is needed - explain}
...
single panel due to \hlite{signal spread},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{what is the signal spread? }
%
another 
...
should be {hlite{as uniform as possible}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{in this context, as uniform as possible means 'any', rephrase}
%
...
example, the 
distributions for all \del{the planes}\add{panels} in 
...
panel illumination. This is due to the 4/4 \del{request}\add{requirement}: 
given the station geometry, the \del{only} 4/4 overlap 
regions are 
limited to the edges of the panels (darkest 
areas in Figure \ref{fig:coordinate}). 
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{replace request --> requirement consistently}

Moreover, the 4/4 \del{request}\add{requirement} selects specific
...
The consequence of the 4/4 \hlite{request} is also a 
...
shows that there \del{are}[add{is} almost no hits in the 
panel central region. This could result in 
waveform non-linearities: \hlite{the signals generated by 
particles which pass close to the edges of 
the straw tend to be wider},
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{depends on the readout end}

\del{and wider signals have greater charge and height compared 
to the signals coming from the 
central part of the wire}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{science fiction, where that came from?}

\hlite{Consequently, the leading 
...
The rate of cosmics \hlite{generated on the horizontal 
  plane}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{generated by whom? }
%
\hlite{above the beam axis}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{doesn't make much sense, fix}
...
which selects \del{the} muons not absorbed in the 
concrete layer posed above the tracker, 
and 0° < $\theta^*$ < 90°. 
If we consider a generation plane of \add{an} area  \add{of} 
...
hit a station is about 8$\times 10^5$ \hlite{and 
this is a good number of events to perform 
a reasonable calibration.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{what consideration this conclusion is based upon?}
...
\begin{itemize}
    ...
    \item the straw midpoint\del{s} $(M_{x,i},M_{y,i})$;
      \todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{'the straw midpoint' is a singular form}
...
\end{aligned}
\end{equation}
The bias \del{on}\add{in} the longitudinal position is the difference between the reconstructed coordinate and the true Monte Carlo position in the panel frame.

\section{Results}
\del{The simulation shows that a resolution \del{on}\add{in} the 
...
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{this statement is wrong and repeated already the second time.
  What is the reasoning the statement is based on?}
...
coordinate of the panel, for panel 0, \del{which may 
be} taken as \del{a good representative for all the panels}\add{an example}.
...
\del{different bumps}\add{hits in different regions in X} correspond to different 
...
\begin{figure}[!h]
    ...
    \label{fig:recx}
\end{figure}
Figure \ref{fig:bias} shows \hlite{the difference   
between the reconstructed track on the panel 
and true longitudinal coordinates}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{a reconstructed track is not a longitudinal coordinate, fix}
...
\begin{figure}[!h]
    ...
    \label{fig:bias}
\end{figure}
The 2D distribution of the \hlite{longitudinal bias}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{introduce a variable for 'longitudinal bias', use the definition}
%
...
to \hlite{visualize the TProfile} of the bias versus 
the true coordinates.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{the meaning is not clear, avoid jargon}
%
...
histogram (CAL side) 
corresponds to the overlap of \hlite{90° panels}.
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{explain what that it}
%
This plot suggests that the mean may not 
serve as an accurate estimator of the bias. 
Under these conditions, \add{achieving the accuracy required for}
the calibration is 
...
\begin{figure}[!h]
    ...
    \label{fig:profile}
\end{figure}
\hlite{The main issue with this type of reconstruction 
lies in the fact that $m_{yz}=\frac{\Delta y}{\Delta z}$ 
is not accurately reconstructed.}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{not true. The main issue is that, for the vertical orientation,
  the stereo reconstruction biases for tracks with positive
  and negative values of dy/dz do not cancel each other}
...
\hlite{The Mu2e collaboration}
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{a) all discussion of a single station calibration has a scope of the tracker VST.
  no point in bringing up the Mu2e collaboration
  b) I suggest to limit this part of conclusions to that with the station positioned vertically,
  the station Vertical Slice Test should be able to achieve (a),(b),(c), ...
  but not (d), which the items expanded on
}

  considered that a vertical station calibration might be feasible for our goals.
...
For this reason, \hlite{a new mechanical approach }
\todo[inline,color=green!20,linecolor=gray,tickmarkheight=10pt]
{whats that?}
%
