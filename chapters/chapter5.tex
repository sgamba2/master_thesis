...
Finally, the last section provides a brief overview of the DAQ performance studies.
  ...
  The test-stand includes the readout chain of an entire 96-channel tracker panel:  
  48 channels on the HV-side and 48 channels on the CAL-side, 1 DRAC board 
  connected via optical fiber to one DTC installed in DAQ computer (mu2edaq09).
...
The steps of the simulation are the following ones:

\begin{itemize}
\item
  The event window starts at $t=0$;
\item
...
As an example, Figure \ref{fig:delay1}(\ref{fig:delay2}) shows the offset between 
...
later part of the readout sequence and may not have all the hits readout, 
...
Figure \ref{fig:66} shows the distribution of the numebr of 
